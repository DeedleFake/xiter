...
func TestBytes(t *testing.T) {
	s := slices.Collect(Bytes("テスト").Seq())
...
func TestRunes(t *testing.T) {
	s := slices.Collect(Runes("これはテストです。").Seq())
...
func TestMapEntries(t *testing.T) {
	s := slices.Collect(ToPair(S2(maps.All(map[string]string{"this": "is", "a": "test"}))).Seq())
...
func TestRecvContext(t *testing.T) {
	...
	s := slices.Collect(RecvContext(context.Background(), c).Seq())
...
func TestStringSplit(t *testing.T) {
	s := slices.Collect(StringSplit("this is a test", " ").Seq())
...
func TestStringFields(t *testing.T) {
	s := slices.Collect(StringFields("  this is a  test ").Seq())
	if !slices.Equal(s, []string{"this", "is", "a", "test"}) {
		t.Fatal(s)
	}
...
func TestSliceChunkBy(t *testing.T) {
	s := slices.Collect(SliceChunksFunc([]int{-1, -2, -3, 1, 2, 3, -1, -2, 3}, func(v int) int { return cmp.Compare(v, 0) }).Seq())
